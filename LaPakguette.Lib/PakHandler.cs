...
namespace LaPakguette.Lib
{
    ...
    public class PakHandler
    {
        ...
        private readonly UnrealPakCmdHelper _unrealPakHelper;
        ...
        public PakHandler(string unrealpakpath)
        {
            ...
            if (!File.Exists(unrealpakpath)) throw new FileNotFoundException();
        ...
        private void SetupAesKeyFromCryptoJson()
        {
            ...
            if (!File.Exists(cryptoPath)) throw new FileNotFoundException();
            var crypto = JsonConvert.DeserializeObject<CryptoModel>(File.ReadAllText(cryptoPath));
        ...
        {
            if (unpackDir == null) unpackDir = pakPath.Replace(".pak", "");
            if (!File.Exists(pakPath)) throw new FileNotFoundException("The .pak file could not be found!");
            var mountpoint = GetMountPoint(pakPath);
            if (mountpoint == null) return false;
            var success = _unrealPakHelper.Unpack(pakPath, unpackDir);
            if (success) CreateRepackFile(pakPath, mountpoint, unpackDir);
            return success;
        }
        ...
        public bool Create(string repackFolder, string pakOutPath = null, bool compress = true, bool encrypt = false,
            bool encryptIndex = true)
        {
            if (pakOutPath == null) pakOutPath = repackFolder + ".pak";
            if (File.Exists(pakOutPath)) Backup(pakOutPath);
        ...
        {
            if (File.Exists(pakPath)) Backup(pakPath);
        ...
        private void Backup(string pakPath)
        {
            ...
            {
                if (File.Exists(pakPath + ".bak")) File.Delete(pakPath + ".bak");
        ...
        private void CreateRepackFile(string pakPath, string mountPoint, string customUnpackDir)
        {
            ...
            var repackFiles = Directory.GetFiles(repackFolderPath)
                .Where(x => Path.GetFileName(x) != _unrealPakHelper.repackFilename);
